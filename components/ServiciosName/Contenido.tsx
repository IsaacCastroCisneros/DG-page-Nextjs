...
export const Contenido = ({data}:{data:any}) => 
{
    ...
    const{total_sesiones,banner,tipo}=program as programData
        ...
        <section>
            <article className="container mx-auto">
                <div className="w-full lg:w-[70%] xl:W-[70%] 2xl:w-[70%] lg:px-10 xl:px-10 2xl:px-10">
                    <p className="text-center text-rojo font-bold text-lg mt-7 flex gap-[.5rem] w-[100%] justify-center"><FontAwesomeIcon size='2xl' icon={faSatelliteDish} /> Este {tipo} contiene {total_sesiones} sesiones de clases en vivo</p>
                    <Image src={banner} width="856" height="347" alt='SIAF' className='mx-auto w-full rounded-md mt-5 h-full lg:h-[260px]' />
                    ...
                    <button className='mx-auto my-10 bg-red-500 p-3 rounded-lg text-white flex items-center gap-3 font-bold swing'>
                        <FontAwesomeIcon size='2xl' icon={faFilePdf} />
                        Descargar Temario PDF</button>
                </div>
            </article>
        </section>
    )
